...
async function handleIssueCredential(request, reply) {
  ...
  try {
    ...
    const {proofPurpose} = options;
    if (proofPurpose !== 'assertionMethod' && proofPurpose !== 'authentication') {
      throw new Error(`${proofPurpose} is not a supported proof purpose, must be 'assertionMethod' or 'authentication'`);
    }
